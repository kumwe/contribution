...
$workspace = rtrim(sys_get_temp_dir(), '/\\') . '/kumwe-contribution-consumer-' . bin2hex(random_bytes(6));
...
consumerRun(
    [
        ...
        '--format=zip',
        '--dir=' . $distDirectory,
        '--file=kumwe-contribution',
    ],
    $workspace,
);
$archive = $distDirectory . '/kumwe-contribution.zip';
if (!is_file($archive)) {
...
$consumerMetadata = [
    ...
    'license' => 'proprietary',
    'require' => ['kumwe/contribution' => $release],
    'repositories' => [
...
$installed = $consumer . '/vendor/kumwe/contribution';
...
foreach (array_keys($classmap) as $symbol) {
    if (
        ...
        || str_starts_with($symbol, 'Kumwe\\Contribution\\Tests\\')
    ) {
