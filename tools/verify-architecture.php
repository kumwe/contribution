...
$source = $root . '/src';
$namespaceRoot = 'Kumwe\\Contribution';
$layers = [
    '' => ['ContributionOwner', 'ContributionDefinition', 'ContributionSurface', 'SurfaceIdentifierPolicy',
        'OwnedContributionRegistry', 'ContributionRejected'],
];
...
if (($composer['name'] ?? null) !== 'kumwe/contribution') {
    $errors[] = 'composer.json must name the package kumwe/contribution.';
}
...
if (($composer['autoload'] ?? null) !== ['psr-4' => [$namespaceRoot . '\\' => 'src/']]) {
    $errors[] = 'composer.json must autoload exactly the one canonical namespace Kumwe\\Contribution\\ from src/.';
}
...
echo 'Architecture verified: ' . count($files)
    . " source files under Kumwe\\Contribution, no host, driver or container coupling.\n";
