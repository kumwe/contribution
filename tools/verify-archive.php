...
foreach ($symbols as $symbol => $entry) {
    ...
    if (
        !is_string($symbol)
        || !str_starts_with($symbol, 'Kumwe\\Contribution\\')
        || !is_string($file)
...
if (($composer['name'] ?? null) !== 'kumwe/contribution') {
    $errors[] = 'The archived composer.json does not name kumwe/contribution.';
}
...
if (($composer['autoload'] ?? null) !== ['psr-4' => ['Kumwe\\Contribution\\' => 'src/']]) {
