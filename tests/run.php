 ...
 *
 * Assertions come from Kumwe\Contribution\Tests\TestCase. No framework, so the suite runs on any supported
...
spl_autoload_register(static function (string $class): void {
    $prefixes = [
        'Kumwe\\Contribution\\Tests\\' => __DIR__ . '/',
        'Kumwe\\Contribution\\' => dirname(__DIR__) . '/src/',
    ];
...
if ($files === []) {
    fwrite(STDERR, "Contribution suite failed: no test case files were discovered.\n");
    exit(1);
}
...
foreach ($files as $file) {
    $class = 'Kumwe\\Contribution\\Tests\\Case\\' . basename($file, '.php');
...
echo "\nContribution suite passed: {$totalTests} tests, {$totalAssertions} assertions.\n";
